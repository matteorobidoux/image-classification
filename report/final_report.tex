...
Accuracy & 0.79 & 0.79 \\ \hline
Train Accuracy & 0.82 & 0.82 \\ \hline
Precision & 0.79 & 0.79 \\ \hline
Recall & 0.79 & 0.79 \\ \hline
F1-Score & 0.79 & 0.79 \\ \hline
Training Time (s) & 0.06 & 0.26 \\ \hline
\end{tabular}
\textbf{\caption{GNB Results}}
\label{gnb_results}
\end{table}
Both models show an identical performance across all metrics, indicating that the custom implementation is working correctly. With an accuracy of \textbf{79\%}, the GNB models perform surprisingly well considering its simplicity and training time.
\newline
\newline
...
Accuracy & 0.55 & 0.60 & 0.57 & 0.57 \\ \hline
Train Accuracy & 0.62 & 0.88 & 1.00 & 1.00 \\ \hline
Precision & 0.56 & 0.61 & 0.57 & 0.57 \\ \hline
Recall & 0.55 & 0.60 & 0.57 & 0.57 \\ \hline
F1-Score & 0.54 & 0.60 & 0.57 & 0.57 \\ \hline
Training Time (s) & 206.57 & 284.12 & 369.03 & 406.09 \\ \hline
\end{tabular}
\textbf{\caption{Custom DT Results with Varying Depths}}
\label{custom_depth_dt_results}
\end{table}
The results show that the depth of the tree has a significant impact on the performance of the custom DT model. With a maximum depth of 5, the model is \textbf{underfitting} the data with a training accuracy of only \textbf{62\%} and a test accuracy of \textbf{55\%}. As the depth increases to 10, the model's performance improves with a training accuracy of \textbf{88\%} and a test accuracy of \textbf{60\%}. However, when the depth is increased to 20 and 50, the model achieves perfect training accuracy but the test accuracy drops to \textbf{57\%}, indicating \textbf{overfitting}. Ultimately, a maximum depth of 10 provides the best outcome with a good balance between training and test accuracy.
...
Accuracy & 0.60 & 0.60 \\ \hline
Train Accuracy & 0.88 & 0.88 \\ \hline
Precision & 0.61 & 0.61 \\ \hline
Recall & 0.60 & 0.60 \\ \hline
F1-Score & 0.60 & 0.60 \\ \hline
Training Time (s) & 284.12 & 0.38 \\ \hline
\end{tabular}
\textbf{\caption{Comparison of Custom and Sklearn DT Results}}
\label{dt_comparison_results}
\end{table}
The results show that both models achieve identical performance across all metrics, indicating that the custom implementation is functioning correctly. However, the training time for the custom DT is significantly higher than that of the Sklearn implementation. This is largely because sckit-learns tree is implemented in optimized C/Cython code with several advanced libraries, while the custom implementation is done purely in Python/NumPy [8]. 
\newline 
\newline
...
Accuracy & 0.82 & 0.84 & 0.82 & 0.80 & 0.83 \\ \hline
Train Accuracy & 0.87 & 0.95 & 1.0 & 1.0 & 1.0 \\ \hline
Precision & 0.83 & 0.85 & 0.82 & 0.81 & 0.83 \\ \hline
Recall & 0.82 & 0.84 & 0.82 & 0.80 & 0.83 \\ \hline
F1-Score & 0.82 & 0.84 & 0.82 & 0.90 & 0.83 \\ \hline
Training Time (s) & 21.25 & 30.49 & 51.95 & 120.66 & 113.82 \\ \hline
\end{tabular}
\textbf{\caption{Custom MLP Results with Varying Architectures}}
\label{custom_mlp_arch_results}
\end{table}
The results show that the increase of depth and width do not always lead to better performance. The shallow architecture had the best overall results with a test accuracy of \textbf{84\%} and a training accuracy of \textbf{95\%}, stipulating that it was able to learn the data well without overfitting. The single-layer MLP also performed well with a test accuracy of \textbf{82\%} and a training accuracy of \textbf{87\%}, showing that even a simple architecture can achieve good results. Where as the deeper and wider models attained \textbf{100\%} training accuracy but had lower test accuracies of \textbf{80\%} and \textbf{83\%} respectively, suggesting overfitting.
\subsubsection{Comparison Between Custom and Sklearn Multi-Layer Perceptron}
Since the shallow architecture provided the best performance for the custom MLP, we will compare it against the Sklearn MLP with the same architecture. The performance metrics for both the custom and Sklearn MLP with the shallow architecture are summarized in Table~\ref{mlp_comparison_results}.
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Metric} & \textbf{Custom MLP} & \textbf{Sklearn MLP} \\ \hline
Accuracy & 0.84 & 0.83 \\ \hline
Train Accuracy & 0.95 & 0.97 \\ \hline
Precision & 0.85 & 0.83 \\ \hline
Recall & 0.84 & 0.83 \\ \hline
F1-Score & 0.84 & 0.83 \\ \hline
Training Time (s) & 30.49 & 6.33 \\ \hline
\end{tabular}
\textbf{\caption{Comparison of Custom and Sklearn MLP Results with Shallow Architecture}}
...
\newline
\newline
To show how the shallow MLP trains over time, a subset of the training logs for the custom implementation is shown in Table~\ref{mlp_training_log}.
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|}
\hline
...
\hline
\end{tabular}
\textbf{\caption{Custom MLP Shallow Architecture Training Log (Subset)}}
\label{mlp_training_log}
\end{table}
The model starts with an accurary of \textbf{48.70\%} in the first epoch and steadily improves over time, reaching a training accuracy of \textbf{92.06\%} by the $100^{th}$ epoch but may have started to overfit near the $75^{th}$ epoch. This indicates that the model is effectively learning from the training data over time.
\newline
\newline
The confusion matrices for both models with the shallow architecture are shown in Figure~\ref{mlp_confusion_matrices}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../results/mlp/mlp_custom/shallow/custom_mlp_confusion_matrix.png}
    \includegraphics[width=0.75\textwidth]{../results/mlp/mlp_sklearn/shallow/sklearn_mlp_confusion_matrix.png}
    \caption{Confusion matrices for the Custom MLP (top) and Sklearn MLP (bottom) with shallow architecture.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../results/cnn/vgg11/custom_cnn/custom_cnn_confusion_matrix.png}
